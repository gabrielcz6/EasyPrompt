...
export default function PromptsPage() {
    ...
    const loadPrompts = async () => {
        try {
            ...
            toast.error(t.prompts.errorLoad);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success(t.prompts.successDelete);
                setPrompts(prompts.filter(p => p.id !== promptToDelete));
            } else {
                toast.error(t.prompts.errorDelete);
            }
        } catch (error) {
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto w-full flex-1">
            <div className="flex items-center justify-between mb-10">
                <h1 className="text-4xl font-extrabold text-foreground tracking-tight">{t.prompts.title}</h1>
            ...
            {loading ? (
                ...
                <div className="text-muted-foreground bg-card p-12 rounded-2xl border border-border shadow-sm text-center flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-lg">{t.prompts.empty}</p>
                                ...
                                <Card className="p-6 h-full bg-card hover:bg-muted/50 border-border hover:border-violet-300 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col rounded-2xl group-hover:border-violet-300">
                                    ...
                                    <div className="text-xs font-medium text-muted-foreground flex justify-between items-center mt-auto pt-4 border-t border-border">
                                        <span className="flex items-center gap-1">
                                            ...
                                            {t.prompts.versions}: <span className="text-muted-foreground font-semibold">{prompt.versions?.length || 0}</span>
                                        </span>
            ...
            <Dialog open={!!promptToDelete} onOpenChange={(open) => !open && setPromptToDelete(null)}>
                <DialogContent className="bg-card border-border rounded-2xl shadow-xl text-foreground">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold text-foreground">{t.prompts.deleteConfirmTitle}</DialogTitle>
                        <DialogDescription className="text-stone-500 pt-3">
                            {t.prompts.deleteConfirmDesc}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="mt-4 flex gap-3">
                        <Button variant="outline" onClick={() => setPromptToDelete(null)} className="border-border hover:bg-muted rounded-lg font-medium text-foreground">{t.common.cancel}</Button>
                        <Button onClick={handleDelete} className="bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold shadow-md">{t.prompts.deleteConfirmBtn}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
